...
//Don't use so much .bss in our kernel, allocate this on the heap instead
portable_samplepair_t (*s_rawsamples)[MAX_RAW_SAMPLES] = NULL;
...
qboolean S_Base_Init( soundInterface_t *si ) {
	...
	// Allocate the raw sample buffers if they haven't been allocated yet
	if (!s_rawsamples) {
	    s_rawsamples = malloc(MAX_RAW_STREAMS * sizeof(*s_rawsamples));
	}
