...
#ifdef __CIRCLE__
#include <stdlib.h>
#define VM_CODE_ALLOC(len) malloc(len)
#define VM_CODE_FREE(ptr, len) (free(ptr), 0)
#define VM_CODE_PROTECT_RX(ptr, len) 0
#else
#include <sys/mman.h>
#define VM_CODE_ALLOC(len) mmap(NULL, (len), PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0)
#define VM_CODE_FREE(ptr, len) munmap((ptr), (len))
#define VM_CODE_PROTECT_RX(ptr, len) mprotect((ptr), (len), PROT_READ|PROT_EXEC)
#endif
...
{
	if (vm->codeBase) {
		if (VM_CODE_FREE(vm->codeBase, vm->codeLength))
...
//	int offsidx = 0;
	...
	if(pass)
	{
		vm->codeBase = VM_CODE_ALLOC(vm->codeLength);
#ifdef __CIRCLE__
		if(vm->codeBase == NULL)
			Com_Error(ERR_FATAL, "VM_CompileARM: can't allocate executable memory");
#else
...
//	offsidx = OFF_IMMEDIATES+1;
	...
	if (VM_CODE_PROTECT_RX(vm->codeBase, vm->codeLength)) {
		VM_Destroy_Compiled(vm);
		DIE("executable memory protect failed");
	}
